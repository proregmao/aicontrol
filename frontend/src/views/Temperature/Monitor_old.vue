...
const temperatureHistory = ref([
  { time: '10:00', temperature: 24.2, humidity: 45 },
  { time: '10:05', temperature: 24.5, humidity: 46 },
  { time: '10:10', temperature: 24.8, humidity: 47 },
  { time: '10:15', temperature: 25.1, humidity: 48 },
  { time: '10:20', temperature: 24.9, humidity: 47 },
  { time: '10:25', temperature: 24.6, humidity: 46 },
  { time: '10:30', temperature: 24.3, humidity: 45 }
])
...
const refreshTemperatureData = () => {
  // 模拟生成新的历史数据
  const now = new Date()
  temperatureHistory.value = Array.from({ length: 7 }, (_, i) => {
    const time = new Date(now.getTime() - (6 - i) * 5 * 60 * 1000)
    return {
      time: time.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' }),
      temperature: Math.round((Math.random() * 3 + 23) * 10) / 10,
      humidity: Math.round(Math.random() * 10 + 40)
    }
  })
}

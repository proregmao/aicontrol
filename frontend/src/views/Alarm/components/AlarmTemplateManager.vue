...
const saveTemplate = async () => {
  ...
  try {
    ...
    if (isEditMode.value) {
      ...
      response = await fetch(`http://localhost:8080/api/v1/alarms/templates/${templateForm.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
      ...
      // 创建新模板
      console.log('➕ 创建新模板')
      response = await fetch('http://localhost:8080/api/v1/alarms/templates', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
...
const deleteTemplate = async (template: any) => {
  try {
    ...
    // 调用API删除模板
    const response = await fetch(`http://localhost:8080/api/v1/alarms/templates/${template.id}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`,
        'Content-Type': 'application/json'

...
// 顺序操作调度器 - 基于讨论的方案实现
// 核心特性：
// 1. 设备级互斥锁（同一断路器同时只能执行一个操作序列）
// 2. 优先级队列（锁定>分合闸>状态检测>参数检测）
// 3. 标准操作序列（参数检测→状态检测→控制操作）
// 4. 异常处理（读取失败继续，控制失败中断）
type SequentialOperationsScheduler struct {
	// 设备级互斥锁
	deviceLocks map[string]*sync.Mutex
	locksMutex  sync.RWMutex
	
	// 优先级队列
	priorityQueues map[int]chan *SequentialOperation // key: priority level
	isRunning      bool
...
type SequentialOperation struct {
	ID          string
	DeviceKey   string // "breaker_1", "breaker_2"
	Device      *Device
	OpType      SequentialOpType
	Action      string
	Priority    int // 1=最高(锁定), 2=高(分合闸), 3=中(状态), 4=低(参数)
...
const (
	SeqOpLockControl    SequentialOpType = "lock_control"    // 锁定/解锁操作
	SeqOpSwitchControl  SequentialOpType = "switch_control"  // 分合闸操作
	SeqOpStatusCheck    SequentialOpType = "status_check"    // 状态检测
	SeqOpParamRead      SequentialOpType = "param_read"      // 参数检测
)
...
type SequentialStats struct {
	...
	InterruptedOps     int // 被高优先级中断的操作
	AverageWaitTime    time.Duration
	AverageExecTime    time.Duration
}

// 创建顺序操作调度器
func NewSequentialOperationsScheduler() *SequentialOperationsScheduler {
	scheduler := &SequentialOperationsScheduler{
...
// 启动调度器
func (s *SequentialOperationsScheduler) Start() {
	s.isRunning = true
	fmt.Println("🚀 顺序操作调度器启动")
	fmt.Println("📋 核心特性:")
	fmt.Println("   - 设备级互斥锁（同一断路器互斥）")
	fmt.Println("   - 优先级队列（锁定>分合闸>状态>参数）")
	fmt.Println("   - 标准操作序列（500ms间隔）")
	fmt.Println("   - 异常处理（读取失败继续，控制失败中断）")
	fmt.Println("   🎯 目标：解决断路器内部操作互斥问题")
...
// 停止调度器
func (s *SequentialOperationsScheduler) Stop() {
	s.isRunning = false
	close(s.stopChan)
	...
	fmt.Println("🛑 顺序操作调度器停止")
}

// 提交操作
func (s *SequentialOperationsScheduler) SubmitOperation(op *SequentialOperation) error {
	if !s.isRunning {
...
// 处理优先级队列
func (s *SequentialOperationsScheduler) processPriorityQueue(priority int) {
...
// 执行顺序操作
func (s *SequentialOperationsScheduler) executeSequentialOperation(op *SequentialOperation) {
	startTime := time.Now()
...
// 获取设备锁
func (s *SequentialOperationsScheduler) getDeviceLock(deviceKey string) *sync.Mutex {
	s.locksMutex.Lock()
...
// 执行锁定控制序列（最高优先级）
func (s *SequentialOperationsScheduler) executeLockControlSequence(op *SequentialOperation) *SequentialResult {
	fmt.Printf("🔒 执行锁定控制序列: %s\n", op.ID)
	
	steps := []StepResult{}
	
	// 步骤1: 锁定状态检测
	step1 := s.executeStep("锁定状态检测", func() (interface{}, error) {
	...
	// 步骤2: 分合闸状态检测
	step2 := s.executeStep("分合闸状态检测", func() (interface{}, error) {
	...
	// 步骤3: 执行锁定操作
	step3 := s.executeStep("执行锁定操作", func() (interface{}, error) {
	...
	// 步骤4: 状态验证
	step4 := s.executeStep("状态验证", func() (interface{}, error) {
		return s.readLockStatus(op.Device)
	})
...
// 执行分合闸控制序列（高优先级）
func (s *SequentialOperationsScheduler) executeSwitchControlSequence(op *SequentialOperation) *SequentialResult {
	...
	// 步骤3: 执行分合闸操作
	step3 := s.executeStep("执行分合闸操作", func() (interface{}, error) {
...
// 执行状态检测序列（中优先级）
func (s *SequentialOperationsScheduler) executeStatusCheckSequence(op *SequentialOperation) *SequentialResult {
...
// 执行参数读取序列（低优先级）
func (s *SequentialOperationsScheduler) executeParamReadSequence(op *SequentialOperation) *SequentialResult {
	fmt.Printf("📈 执行参数读取序列: %s\n", op.ID)

	steps := []StepResult{}

	// 步骤1: 参数检测（电压、电流、温度）
	step1 := s.executeStep("参数检测", func() (interface{}, error) {
	...
	// 步骤2: 锁定状态检测
	step2 := s.executeStep("锁定状态检测", func() (interface{}, error) {
		return s.readLockStatus(op.Device)
	})
	steps = append(steps, step2)
	...
	// 步骤3: 分合闸状态检测
	step3 := s.executeStep("分合闸状态检测", func() (interface{}, error) {
		return s.readSwitchStatus(op.Device)
	})
	steps = append(steps, step3)
...
// 执行单个步骤
func (s *SequentialOperationsScheduler) executeStep(stepName string, stepFunc func() (interface{}, error)) StepResult {
	startTime := time.Now()
	fmt.Printf("  🔹 执行步骤: %s\n", stepName)
...
// 获取连接
func (s *SequentialOperationsScheduler) getConnection(device *Device) (net.Conn, error) {
	...
	// 检查现有连接
	if conn, exists := s.connections[key]; exists && conn != nil {
		return conn, nil
	}
...
// 读取设备参数（电压、电流、温度）
func (s *SequentialOperationsScheduler) readDeviceParams(device *Device) (map[string]interface{}, error) {
	conn, err := s.getConnection(device)
	if err != nil {
		return nil, err
	}
...
// 读取锁定状态
func (s *SequentialOperationsScheduler) readLockStatus(device *Device) (map[string]interface{}, error) {
	conn, err := s.getConnection(device)
	if err != nil {
		return nil, err
	}
...
// 读取分合闸状态
func (s *SequentialOperationsScheduler) readSwitchStatus(device *Device) (map[string]interface{}, error) {
	conn, err := s.getConnection(device)
	if err != nil {
		return nil, err
	}
...
// 执行锁定操作
func (s *SequentialOperationsScheduler) executeLockOperation(device *Device, action string) (map[string]interface{}, error) {
	conn, err := s.getConnection(device)
	if err != nil {
		return nil, err
	}
...
// 执行分合闸操作
func (s *SequentialOperationsScheduler) executeSwitchOperation(device *Device, action string) (map[string]interface{}, error) {
	conn, err := s.getConnection(device)
	if err != nil {
		return nil, err
	}
...
// 读取输入寄存器
func (s *SequentialOperationsScheduler) readInputRegister(conn net.Conn, deviceAddr uint8, address uint16) (uint16, error) {
	...
	time.Sleep(50 * time.Millisecond) // 等待响应
...
// 写单个线圈
func (s *SequentialOperationsScheduler) writeSingleCoil(conn net.Conn, deviceAddr uint8, address uint16, value uint16) error {
	request := make([]byte, 12)
...
// 更新统计信息
func (s *SequentialOperationsScheduler) updateStats(op *SequentialOperation, result *SequentialResult, waitTime time.Duration) {
	s.statsMutex.Lock()
...
// 获取统计信息
func (s *SequentialOperationsScheduler) GetStats() SequentialStats {
	s.statsMutex.RLock()
...
// 主测试程序
func main() {
	fmt.Println("🧪 顺序操作调度器测试")
	fmt.Println("====================================================")
	fmt.Println("📋 测试方案（基于讨论结果）:")
	fmt.Println("   - 设备级互斥锁（同一断路器互斥）")
	fmt.Println("   - 优先级队列（锁定>分合闸>状态>参数）")
	fmt.Println("   - 标准操作序列（500ms/1000ms间隔）")
	fmt.Println("   - 异常处理（读取失败继续，控制失败中断）")
	fmt.Println("   🎯 目标：解决断路器内部操作互斥问题")
	fmt.Println()
	...
	// 创建调度器
	scheduler := NewSequentialOperationsScheduler()
	scheduler.Start()

	time.Sleep(1 * time.Second) // 启动等待

	fmt.Println("📋 开始顺序操作测试...")
	...
	testOperations := []struct {
		...
		// 场景1: 参数检测（低优先级）
		{SeqOpParamRead, breaker1, "", 4, "断路器1参数检测"},
		{SeqOpParamRead, breaker2, "", 4, "断路器2参数检测"},

		// 场景2: 状态检测（中优先级）
		{SeqOpStatusCheck, breaker1, "", 3, "断路器1状态检测"},
		{SeqOpStatusCheck, breaker2, "", 3, "断路器2状态检测"},

		// 场景3: 分合闸操作（高优先级）
		{SeqOpSwitchControl, breaker1, "close", 2, "断路器1合闸操作"},
		{SeqOpSwitchControl, breaker2, "close", 2, "断路器2合闸操作"},

		// 场景4: 锁定操作（最高优先级）
		{SeqOpLockControl, breaker1, "lock", 1, "断路器1锁定操作"},
		{SeqOpLockControl, breaker2, "unlock", 1, "断路器2解锁操作"},

		// 场景5: 混合操作测试（验证优先级）
		{SeqOpParamRead, breaker1, "", 4, "断路器1参数检测（混合测试）"},
		{SeqOpSwitchControl, breaker1, "open", 2, "断路器1分闸操作（混合测试）"},
		{SeqOpStatusCheck, breaker2, "", 3, "断路器2状态检测（混合测试）"},
		{SeqOpLockControl, breaker2, "lock", 1, "断路器2锁定操作（混合测试）"},
	}
	...
	for i, testOp := range testOperations {
		...
		go func(index int, ch chan *SequentialResult) {
			...
			case <-time.After(60 * time.Second): // 给足够的时间执行序列
				fmt.Printf("⚠️ 操作超时: seq-op-%d\n", index+1)
			}
		}(i, responseChan)

		time.Sleep(200 * time.Millisecond) // 提交间隔
	}

	// 等待所有操作完成
	expectedTime := float64(len(testOperations)) * 3.0 // 每个操作预计3秒
	...
	for i, result := range responses {
		if result != nil {
			...
			for _, step := range result.Steps {
				...
				fmt.Printf("    %s %s (耗时: %v)\n", stepStatus, step.StepName, step.Duration)
			}
		} else {
	...
	if successRate >= 90 {
		fmt.Println("🎉 测试优秀通过！")
		fmt.Println("   ✅ 设备级互斥锁工作正常")
		fmt.Println("   ✅ 优先级队列调度有效")
		fmt.Println("   ✅ 标准操作序列执行成功")
		fmt.Println("   ✅ 异常处理策略有效")
		fmt.Println("   🚀 可以集成到生产系统")
	} else if successRate >= 70 {
		fmt.Println("✅ 测试良好通过")
		fmt.Printf("   - 成功率: %.1f%% (良好水平)\n", successRate)
		fmt.Println("   - 核心功能验证成功")
		fmt.Println("   🚀 可以进一步优化后集成")
	} else {
		fmt.Println("⚠️ 测试需要改进")
		fmt.Printf("   - 成功率: %.1f%% (期望: ≥90%%)\n", successRate)
		fmt.Println("   - 建议检查网络连接和设备状态")
	}

	fmt.Println("\n✅ 顺序操作调度器测试完成!")
	fmt.Println("📋 基于讨论方案的断路器内部操作互斥问题解决方案验证完成")
}
